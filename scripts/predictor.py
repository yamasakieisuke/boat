...
#
# 0.10 では**足りなかった**。頭ごとの2着候補が入れ替わったのは4,130組中16組
# （0.4%）だけで、買い目が変わっても2着の中身は動いていなかった。
# 2着候補間のスコア差は 0.02〜0.05 あるのに、±0.028 では届かない。
#
# 0.20 で ±0.056。cond2 の寄与（0.1〜0.5 × 0.35 = 0.035〜0.175）と同程度になる。
# 実測の差は大きい（3コースまくり型なら 2着=4 が周辺19.4%→27.5%、
