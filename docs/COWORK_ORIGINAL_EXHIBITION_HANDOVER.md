# Cowork 引き継ぎ：福岡オリジナル展示の取得（2026-05-01 更新）

**重要：2026-05-01 に boat-race-fetcher の4分間隔ポーリングを停止**。月$2,500ペースのコストを削減するため、ユーザー指示時のみ `python3 scripts/run_pending.py --jcd 22` で福岡だけ実行する運用に変更。以下の本文は当時の常時ポーリング前提の記述が残っているが、現運用では「指定会場のみ・対話呼び出し」で動く。

---

# Cowork 引き継ぎ：福岡オリジナル展示の取得（2026-04-29 当初版）
