---
description: boatリポジトリで使えるコマンド・ドキュメント・運用ルールの一覧を表示（/sync /pull 等）
---
...
| コマンド | 動作 | 使うタイミング |
|---|---|---|
| `/sync` | `git add -A` + `git commit` + `git push` を一発実行 | 作業区切り・作業終了時 |
| `/pull` | `git pull --rebase --autostash` | 他端末変更後の手動最新化 |
| `/menu` | このメニューを表示 | コマンド・ドキュメント確認時 |
| `/help` | Claude Code 組み込みヘルプ（別物） | - |
...
⚠️ **補足**: `/help` は Claude Code 組み込みで上書き不可。カスタムメニューは `/menu` を使用。
